...
=pod
spider.pm
Create By takamitu hamada
http://nightmare.sourceforge.jp
ver:20110425
=cut
package spider;
use strict;
...
sub arrayForJS{
	my $self = shift;
	my ($filedir,$arrayname) = @_;
	my $obj = spider->new;
	my $ord = $obj->read_dir($filedir);
	my $str = "var ".$arrayname."="."[".$ord."];\n";
	return $str;
}
sub loadRSS{
	my $self = shift;
	my ($url) = @_;
	my $document = LWP::Simple::get($url) || die "Can't get content from $url";
	my $xml = XML::Simple->new;
	my $data = $xml->XMLin($document);
	my $title;
	my $description;
	if($url =~ /\.rdf$/ || $url =~ /index.xml$/){
		foreach my $item($data->{item}){
			my $entry = $data->{item}->{$item};
			$title .= "<h1>".$entry->{title}."</h1>\n";
			$description .= "<p>".$entry->{description}."</p>\n";
		}
	}elsif($url =~ /atom.xml$/){
		foreach my $item(sort{$b cmp $a}keys % {$data->{entry}}){
			my $entry = $data->{entry}->{$item};
			$title .= "<h1>".$entry->{title}."</h1>\n";
			$description .= "<p>".$entry->{content}->{content}."</p>\n";
		}
	}
...
sub RSS2List{
	my $self = shift;
	my ($url) = @_;
	my $document = LWP::Simple::get($url) || "Can't get content from $url";
	my $xml = XML::Simple->new;
	my $data = $xml->XMLin($document);
	my $title;
	my $description;
...
sub firefoxBookmark{
	my $self = shift;
	my ($html) = HTML::TagParser->new(@_);
	my @list = $html->getElementsByTagName("a");
	foreach my $elem(@list){
		my $ga = $elem->getAttribute("href");
		my $text = $elem->innerText();
		print "<li><a href='".$ga."' target='_blank'>".$text."</a><br/>";
	}
}
sub HTMLinfo{
	my $self = shift;
	my ($html) = @_;
	my $str;
	my $hp = new HTML::HeadParser;
	$hp->parse($html);
	my %parse = (
		"title" => $hp->header("title"),
		"charset" => $hp->header("content-type"),
		"copyset" => $hp->header("x-meta-copyright"),
		"kewwords" => $hp->header("x-meta-keywords"),
		"description" => $hp->header("x-meta-description")
	);
